...
function gradesAvg(arr) {
    ...
    const reVal=arr.map(({firstName,lastName,gradsList})=>{
        let average =0;
        for(let i=0;i<gradsList.length;i++){
            average+=gradsList[i];
        }
        average=average/gradsList.length;
        return {firstName,lastName,gradsList, avg:average}
...
function studentsResult(arr) {
    // write your code here
    const reVal=arr.map(({firstName,lastName,gradsList,avg})=>{
        let res="";
        if(avg>=50){
            res="Passed";
            }else{
        ...
        return {firstName,lastName,gradsList, avg,result:res}
