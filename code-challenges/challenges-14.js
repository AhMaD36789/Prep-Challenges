...
const LastWord = (str) => {
    ...
    let returnVal = str.slice(lastSpace + 1);
    return returnVal;
...
const replaceWords = (str) => {
    // write your code here
    let mappingStr = str.split(" ");
    mappingStr.map((item, index) => {
        if (item == "I") {
            mappingStr.splice(index, 1, "We")
        } else if (item == "am") {
            mappingStr.splice(index, 1, "are")
        } else if (item == "was") {
            mappingStr.splice(index, 1, "were")
        }
    });
    let returnVal = mappingStr.toString();
